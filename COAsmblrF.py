...
def var_form(name):
    global var
    global counter
    sum = str(bin(counter).replace("0b",""))
    while(len(sum)<8):
        sum="0"+sum
...
def label_form(name,labelline):
    global label
    sum = str(bin(labelline).replace("0b",""))
    while(len(sum)<8):
        sum="0"+sum
...
def checkC(data,line_num):
    ...
    for i in range(0,len(data)):
        ...
        elif (data[i]=="FLAGS" and truestr=="ORR"):       # flag error udated
            print(f"Line {line_num}:Misuse of FLAGS Error")
            quit()
            finalflag=1
            errorflag=1
    ...
    if(checkstr==truestr):
        ...
        for i in data:
            if i in opcode_C:
                converted+=str(int(opcode_C[i]))+"00000"
            elif i in reg:
                converted+=reg[i]
            elif i in Flags:
                converted+=Flags[i]
    ...
    if(checkstr==truestr):
        ...
        for i in data:
            if i =="movf":
                ...
                # check for immediate >=0 and <=255
                immbin,flocount = binconvo(float(i[1:len(i)]))
                imm=checkValid(immbin,flocount)
                # imm=str(bin(int(i[1:len(i)])).replace("0b",""))  
                if (imm==False):
                    print(f"Line {line_num}:Not a valid Float Immediate ")
                    quit()
                    finalflag=1
                    immediateflag=1
                    errorflag=1
...
# TO cut variables from top and paste to bottom
length=len(list1)
Variablelist=[]
for i in range(length):
    tempdata=list1[i].split()
    try:
        if(tempdata[0]=="var"):
            Variablelist.append(list1[i])
            list1[i]=""
        else:
            break
    except:
    ...
    try: 
        if(data[0][-1]==":"):
            ...
            if(doublelabelflag!=1 and len(data)>1):    
                # print("Form label")
                label_form(data[0][0:len(data[0])-1],labelcount)
    except:
        # print("pass")
        pass
...
elif data[0]=="hlt" and len(data)>1:        
    print("NO hlt present at end or hlt syntax is not appropriate")
    quit()
...
for i in range(len(list1)):
    ...
    elif(errorflag!=1 and data[0]=="mov" and data[1]=="FLAGS"):
        if (data[2]=="FLAGS"):
            print(f"Line {i+1}:Misuse of FLAGS Error")
            quit()
            errorflag=1
            finalflag=1
        
        if(errorflag!=1 and checkC(data,i+1)==True):
            finallist.append(convert1)
            flagvar=1
    
    elif(errorflag!=1 and checkA(data,i+1)==True): 
        
        
        finallist.append(convert1)
        flagvar=1

    elif(errorflag!=1 and checkC(data,i+1)==True):
        
        
        finallist.append(convert1)
        flagvar=1
    elif(errorflag!=1 and checkB(data,i+1)==True):
        
        
        finallist.append(convert1)
        flagvar=1

    elif(errorflag!=1 and checkD(data,i+1)==True):
        
        
        finallist.append(convert1)
        flagvar=1

    elif(errorflag!=1 and checkE(data,i+1)==True):
        
        
        finallist.append(convert1)
        flagvar=1
# float
    elif(errorflag!=1 and data[0]=="addf"):
        # print("ERR ",errorflag)
        if(errorflag!=1 and addf(data,i+1)==True):
            # print("IN ADDF")
            finallist.append(convert1)
            flagvar=1

    elif(errorflag!=1 and data[0]=="subf"):
        if(errorflag!=1 and subf(data,i+1)==True):
            # print("IN SUBF")
            finallist.append(convert1)
            flagvar=1

    elif(errorflag!=1 and data[0]=="movf"):
        if(errorflag!=1 and movf(data,i+1)==True):
            # print("IN MOVF")
            finallist.append(convert1)
            flagvar=1   
        ...
        if len(data)>1:
            ...
            print(f"Line {i+1}:Multiple Halt detected")
            quit()
            finalflag=1
    elif(errorflag!=1):
        print(f"Line {i+1}:General Syntax Error")
